...
class TrackerService:
    def __init__(self, host: str = '0.0.0.0', port: int = 6969):
        self.host = host
        self.port = port
        self.peers: Dict[bytes, Set[Tuple[str, int]]] = {}  # info_hash -> set of (ip, port)
        self.last_announce: Dict[Tuple[bytes, str, int], float] = {}  # (info_hash, ip, port) -> timestamp
    ...
    async def handle_announce(self, request):
        ...
        try:
            ...
            logger.info(f"Announce from {client_ip}:{port} for info_hash {info_hash.hex()}")
            ...
            if event in ('started', '', 'completed', None):
                ...
                logger.info(f"Added peer {client_ip}:{port} for info_hash {info_hash.hex()}")
            elif event == 'stopped':
                self.peers[info_hash].discard((client_ip, port))
                logger.info(f"Removed peer {client_ip}:{port} for info_hash {info_hash.hex()}")
            ...
            response_data = {
                ...
                b'complete': len([p for p in peers_list if p[1] == 100]),  # Fake seeder count
