...
class MethodMapper:
        ...
        try:
            ...
            if method.upper() == "GET":
                ...
                response = self.session.request(
                    method.upper(), url, json=data, timeout=30
                )
        ...
        for torrent in torrents_data:
            ...
            for method_call in args[1:]:
                ...
                try:
                    ...
                    logger.error(
                        f"Error processing {method_clean} for {info_hash}: {e}"
                    )
    ...
    def _map_method_to_field(self, method: str, torrent: Dict) -> Any:
        ...
        mapping = {
            ...
            "d.custom=addtime": str(torrent.get("added_time", "")),
            # Size and progress
    ...
    def _map_file_method(self, method: str, file_info: Dict, index: int) -> Any:
        ...
        mapping = {
            ...
            "f.size_chunks": file_info.get("size_chunks", 0),
    ...
    # Peer methods
    def p_multicall(self, info_hash: str, *args: str) -> List[List[Any]]:
        """Peer multicall"""
        torrent_data = self.call_backend(f"/api/torrents/{info_hash}", "GET")
        data = torrent_data.get("peers", []) if torrent_data else []
    ...
    def _map_peer_method(self, method: str, peer: Dict) -> Any:
        """Map peer methods to peer data"""
        mapping = {
            "p.address": peer.get("address", ""),
            "p.down_rate": str(peer.get("down_rate", 0)),
            "p.up_rate": str(peer.get("up_rate", 0)),
            "p.is_incoming": peer.get("is_incoming", False),
        }
    ...
    def t_multicall(self, info_hash: str, *args: str) -> List[List[Any]]:
        """Tracker multicall"""
        trackers_data = self.call_backend(f"/api/torrents/{info_hash}", "GET")
    ...
    def load_start(self, data: str, *args) -> int:
        ...
        if data.startswith("magnet:"):
            ...
            result = self.call_backend(
                "/api/torrents", "POST", {"torrent": torrent_b64}
            )
