...
@logged
class SeedHandler:
    """Handles the seeding phase of a connection"""
    ...
    def run(self):
        ...
        try:
            ...
            self.protocol.send_bitfield(bitfield)
            ...
            while True:
                ...
                if msg_id == PeerWireProtocol.CHOKE:
                    ...
                    # Unchoke the peer if they're interested
                    ...
                    # Peer has a new piece, but we don't need it as a seeder
                    pass
                elif msg_id == PeerWireProtocol.BITFIELD:
                    # Peer sent their bitfield, but we don't need it as a seeder
                    pass
                elif msg_id == PeerWireProtocol.CANCEL:
                    # Peer canceled a request, but we don't handle requests that way
                    pass
    ...
    def _handle_request(self, payload: bytes):
        """Handle a request for a piece block"""
        ...
        # Send the block
        self.protocol.send_piece(index, begin, block)
        self.logger.debug(f"Sent piece {index}, begin {begin}, length {length} to {self.protocol.peer}")
